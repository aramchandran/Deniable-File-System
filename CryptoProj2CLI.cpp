...
//DECRYPT BLOCK
void decrypt(unsigned char indata[256], int blcksize, unsigned char * ckey, FILE *op)
{
	fseek(op, 0, SEEK_END);
	AES_set_encrypt_key(ckey, blcksize, &key);
	unsigned char outdata[256]= {};
...
//HASH CHECKER
void hash_checker(unsigned char buffer[256], unsigned char indata[256], unsigned char * key, int blcksize, int j, FILE *op)
{
	int flag = 0;
	unsigned char *result = NULL;
	unsigned int len = 64;
	...
	if (flag == 1)
	{
		decrypt(indata, blcksize, (unsigned char *)key, op);
		free(result);
	}
	else
	{
...
int main()
{
	...
	else if (cs == 2)
	{
		...
		for (i = 0; i < num_blocks; i++)
		{
			...
			hash_checker(buffer, buffer1, ckey, blcksize, j, op);
		}
		fclose(op);
		remove(filename);
